...
namespace IT3_Degradation
{
    public partial class Main : Form
    {
        ...
        private void DrawButton_Click(object sender, EventArgs e)
        {
            ...
            for_draw = new Autoregression(ampArray, freqArray, amount, noise, sizeM, size, edge);

            signal = new double[size];
            signal = for_draw.CreateGraph(signal);

            signal = for_draw.AddNoise(signal);

            autoReg = for_draw.Calc(signal);
        ...
        private void FoureaButton_Click(object sender, EventArgs e)
        {
            using(Fourea form = new Fourea(for_draw.sourceModule, for_draw.predictedModule))
            {
                form.ShowDialog();
            }
        }
